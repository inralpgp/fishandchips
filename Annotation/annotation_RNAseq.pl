...
$species=$ARGV[0];
$blast_type=$ARGV[1];	# "Megablast" or "Discontiguous"
...
if($existing_contig==1)
{
	...
	$retrieve_contig2=$dbh->prepare("
		...
		FROM `Annotation_all_sequences` , `annotations_".$spe_DB."`
		WHERE `Annotation_all_sequences`.`seq_id` = `annotations_".$spe_DB."`.`seq_id`
		...
		AND `annotations_".$spe_DB."`.`blast_type` IS NULL;
	");
