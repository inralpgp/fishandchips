...
foreach $file (@files)
{
	...
	if(-e "$dir/$gse/$file/cluster/cluster_1.txt")
	{
		...
		opendir DIR2, "$dir/$gse/$file/cluster/";
		my @list_clusters = () ;
		...
		foreach $cluster (sort @list_clusters)
		{
		  ...
		  system("R --vanilla --slave < ./7-cluster_quality/test_outliers_et_bruit.R > $dir/$gse/$file/cluster/$cluster_file.R.log")== 0 or print STDERR "R script failed";
			my $test1 = 0 ;
			if(-e "$dir/$gse/$file/cluster/$cluster_file.R.log")
			{
			
				open(TOTO, "$dir/$gse/$file/cluster/$cluster_file.R.log") or die "impossible d'ouvrir $cluster_file.txt\n" ;
				while(<TOTO>)
				{
					chomp;
					if($_ =~ /^pvalue=/) 
